...
class MultiAgentsPPOTrainer:
    ...
    def init_workers(self):
        """Initialize workers for all PPO trainers in parallel (synchronous wrapper)."""
        asyncio.run(self._init_workers_async())
    
    async def _init_workers_async(self):
        """Initialize workers for all PPO trainers asynchronously in parallel."""
        ...
        async def init_single_trainer(model_name, trainer):
            """Initialize workers for a single trainer."""
            # Run the synchronous init_workers in a thread pool
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(None, trainer.init_workers)
            colorful_print(f"Initialized workers for trainer: {model_name}", "green")
        
        # Run all trainer initializations in parallel
        tasks = [
            init_single_trainer(model_name, trainer) 
            for model_name, trainer in self.ppo_trainer_dict.items()
        ]
    ...
    async def _update_all_parameters_parallel(self, batch_per_trainer, gen_batch_output_per_policy, timing_raw):
        """
        Parallel execution of parameter updates for all trainers.
        Returns: Dictionary of all trainer metrics
        """
        all_trainer_metrics = {}
        
        # Prepare async tasks for parallel parameter updates
        update_tasks = []
        for model_name, trainer in self.ppo_trainer_dict.items():
            ...
            task = self._update_parameters_async(
                model_name,
        ...
        # Execute all parameter updates in parallel
        if update_tasks:
            colorful_print(f"Updating parameters for {len(update_tasks)} trainers in parallel...", "cyan")
            completed_models = await asyncio.gather(*update_tasks)
            colorful_print(f"Parameter updates completed for models: {completed_models}", "green")
        
        # Collect metrics from each trainer's batch (after async updates are done)
        for model_name, trainer in self.ppo_trainer_dict.items():
            if model_name not in gen_batch_output_per_policy:
                continue
        ...
        return all_trainer_metrics

    async def _update_parameters_async(self, model_name, batch, ppo_trainer, timing_raw):
        """Async wrapper for _update_parameters to enable parallel execution."""
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, self._update_parameters, batch, ppo_trainer, timing_raw)
        return model_name
        ...
        for i,step in enumerate(progress_bar):
            ...
            with simple_timer("step", timing_raw):
                ...
                with simple_timer("update_parameters", timing_raw):
                    ...
                    # Use asyncio.run to execute parallel parameter updates
                    all_trainer_metrics = asyncio.run(self._update_all_parameters_parallel(
                        batch_per_trainer, 
                ...
                if self.config.trainer.save_freq > 0 and self.global_steps % self.config.trainer.save_freq == 0 and self.global_steps != 0 and self.global_steps != 1:
