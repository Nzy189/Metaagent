...
model_0_resource="resource.n_gpus_per_node=$GPU_num \
  ...
  $model_0_config_path.actor_rollout_ref.rollout.tensor_model_parallel_size=2"

rollout_memory_fix="$model_0_config_path.actor_rollout_ref.rollout.max_num_seqs=32 \
  $model_0_config_path.actor_rollout_ref.rollout.gpu_memory_utilization=0.4 \
  $model_0_config_path.actor_rollout_ref.rollout.max_num_batched_tokens=32768 \
...
python3 -m pettingllms.trainer.train --config-path ../config/math --config-name math_L2_lora \
    ...
    training.experiment_name=math_1.7B_lora_4gpu\
